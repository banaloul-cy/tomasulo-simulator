...
import { FP_REGISTERS, BASE_REGISTERS } from '../core/config.js';
...
export function stationVisualState(state, station) {
  const broadcastThisCycle = state.cdbLog.some(
    (e) => e.cycle === state.cycle && e.station === station.name
  );
...
function renderStationPool(tbody, stations, state, kind) {
  ...
  for (const s of stations) {
    const visual = stationVisualState(state, s);
    const tr = document.createElement('tr');
    const rowClass = rowClassFor(visual);
...
function renderRegisters(state) {
  ...
  const rows = [...FP_REGISTERS, ...BASE_REGISTERS];
  for (const reg of rows) {
    ...
    if (isBase) {
      ...
      const qi = state.registerStatus[reg];
...
function renderCdbLog(state) {
  ...
  const entries = [...state.cdbLog].reverse();
